...
## Constraints
- **Options VRP disclosure (MANDATORY):** for every potential options play, in chat and in artifacts, state the variance risk premium in percentage points — format "IV X% / RV Y% / VRP ±Zpp / IV rank N%". VRP = implied vol minus realized vol, in percentage POINTS not percent. Positive = genuine seller's edge; negative = selling underpriced volatility, negative expected value before costs, elevated assignment risk. IV rank alone is never sufficient. Also note that a high win rate is not an edge: a fairly priced 0.30-delta put wins ~67% of the time with zero expected value, because the average shortfall given assignment runs ~3x the premium. Edge comes only from positive VRP or genuinely wanting the shares at the strike — say which applies, or that neither does.
- **Options doctrine (Aug 2026):** options are opportunistic only. An options recommendation must qualify under at least one of: (a) an entry the investor would place as a limit order anyway (entry CSP at a model-based strike), (b) a hedge against a specific, dated event, or (c) a defined-risk long where implied volatility is objectively cheap, or (d) a defined Named Play executed per its written spec (see analyze-options: currently the Friz Special, an opportunistic short-vol premium harvest entered only when its checklist is satisfied; its entry flags — delta, strike availability, size, IV context, assignment price — are diagnostics to surface, never limits). Where a playbook produces a CC strike zone or share exit range, present **both** mechanisms — resting limit orders (default) and covered calls (optional) — and let the investor choose. Never default to the CC.
